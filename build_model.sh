...
if [ -f "~/resource_model/res_data" ]; then
  rm ~/resource_model/res_data
fi
...
for i in {20..100..1}
  do
    ...
    slo=$(cat ~/RUBiS/${perf_path}perf.html | grep '<TR><TD><div align=left><B>Total</div></B><TD><div align=right>' | egrep -o [0-9]+ | sed -n '10 p')
    ssh "$kvm_host_user@$kvm_host_ip" "sudo kill -9 \$(ps aux | grep monitor_pressure.sh | awk '{ print \$2 }')"
    scp $kvm_host_user@$kvm_host_ip:/home/$kvm_host_user/resource_model/pressure_data ~/resource_model/pressure_data
    pressure=$(python ~/resource_model/calculate_pressure.py)
    echo "$pressure $slo" >> ~/resource_model/res_data
    if [ "$slo" -lt 200 ]
      then
